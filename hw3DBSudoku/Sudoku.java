...
public class Sudoku {
	...
	private void creatSpots(){
		for(int i = 0; i < board.size(); i++){
			int cur = 0;
			Spot s = board.get(i);
			if(s.get() == 0) cur = s.getValues(board, i).size();
			int index = 0;
			for(int j = 0; j < spots.size(); j++){
				if(spots.get(i).getValue() > cur){
					index = j;
					break;
				}
			}
